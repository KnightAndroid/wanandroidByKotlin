...
import com.knight.kotlin.library_permiss.permission.PermissionLists.getReadMediaVisualUserSelectedPermission
...
/**
 * @Description 读取视频媒体权限类
 * @Author knight
 * @Time 2025/7/10 21:42
 *
 */

class ReadMediaVideoPermission : DangerousPermission {
    constructor()
    ...
    override fun getOldPermissions(context: Context): List<IPermission> {
        ...
        return PermissionUtils.asArrayList(getReadExternalStoragePermission())
    }

    override fun isGrantedPermissionByStandardVersion( context: Context, skipRequest: Boolean): Boolean {
        if (isAndroid14() && !skipRequest) {
            ...
            return getReadMediaVisualUserSelectedPermission().isGrantedPermission(context, false)
        }
    ...
    protected override fun checkSelfByRequestPermissions( activity: Activity,  requestList: List<IPermission>) {
        ...
        require(!PermissionUtils.containsPermission(requestList, PermissionNames.READ_EXTERNAL_STORAGE)) {
    ...
    companion object {
        val PERMISSION_NAME: String = PermissionNames.READ_MEDIA_VIDEO
        @JvmField
        val CREATOR : Parcelable.Creator<ReadMediaVideoPermission> =


            object : Parcelable.Creator<ReadMediaVideoPermission> {
                /** 当前权限名称，注意：该常量字段仅供框架内部使用，不提供给外部引用，
                 * 如果需要获取权限名称的字符串，请直接通过 [PermissionNames] 类获取
                 */

                override fun createFromParcel(source: Parcel): ReadMediaVideoPermission {
                    return ReadMediaVideoPermission(source)
                }

                override fun newArray(size: Int): Array<ReadMediaVideoPermission?> {
