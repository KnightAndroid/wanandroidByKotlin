...
object PermissionIntentManager {
    /** 华为手机管家 App 包名  */
    private const val EMUI_MOBILE_MANAGER_APP_PACKAGE_NAME: String = "com.huawei.systemmanager"

    /** 小米手机管家 App 包名  */
    private const val MIUI_MOBILE_MANAGER_APP_PACKAGE_NAME: String = "com.miui.securitycenter"

    /** OPPO 安全中心 App 包名  */
    private const val COLOR_OS_SAFE_CENTER_APP_PACKAGE_NAME_1: String = "com.oppo.safe"
    private const val COLOR_OS_SAFE_CENTER_APP_PACKAGE_NAME_2: String = "com.color.safecenter"
    private const val COLOR_OS_SAFE_CENTER_APP_PACKAGE_NAME_3: String = "com.oplus.safecenter"

    /** vivo 安全中心 App 包名  */
    private 
    const val ORIGIN_OS_MOBILE_MANAGER_APP_PACKAGE_NAME: String = "com.iqoo.secure"

    /** 锤子安全中心包名  */
    private 
    const val SMARTISAN_OS_SECURITY_CENTER_APP_PACKAGE_NAME: String =
        "com.smartisanos.securitycenter"

    /** 锤子安全组件包名  */
    private 
    const val SMARTISAN_OS_SECURITY_COMPONENT_APP_PACKAGE_NAME: String = "com.smartisanos.security"
    ...
    fun getEmuiWindowPermissionPageIntent(context: Context): Intent? {
        ...
        // 获取厂商版本号
        var romVersionName = getRomVersionName()
    ...
    fun getSmartisanWindowPermissionPageIntent(context: Context): Intent? {
    ...
    fun getSmartisanPermissionPageIntent(context: Context): Intent? {
        var intent: Intent? =
            Intent("$SMARTISAN_OS_SECURITY_COMPONENT_APP_PACKAGE_NAME.action.PACKAGE_OVERVIEW")
        if (PermissionUtils.areActivityIntent(context, intent)) {
            return intent
        }
        ...
        intent = Intent()
        intent.setClassName(
            SMARTISAN_OS_SECURITY_COMPONENT_APP_PACKAGE_NAME,
            "$SMARTISAN_OS_SECURITY_COMPONENT_APP_PACKAGE_NAME.PackagesOverview"
        )
        if (PermissionUtils.areActivityIntent(context, intent)) {
            return intent
        }
        ...
        intent = context.packageManager.getLaunchIntentForPackage(
            SMARTISAN_OS_SECURITY_CENTER_APP_PACKAGE_NAME
        )
        if (PermissionUtils.areActivityIntent(context, intent)) {
            return intent
        }
    ...
    fun getApplicationDetailsIntent(
         context: Context,
        ...
        if (permissions != null && !permissions.isEmpty() && isColorOs()) {
            ...
            bundle.putStringArrayList(
                "permissionList",
                if (permissions is ArrayList<*>) permissions as ArrayList<String?> else ArrayList(
                    permissions
                )
            )
