...
class RequestPermissionLogicPresenter(
    ...
    fun request() {
        ...
        val unauthorizedPermissions: List<List<IPermission?>> =
            getUnauthorizedPermissions(mActivity, mRequestPermissions)
        ...
        val iterator: Iterator<List<IPermission?>> = unauthorizedPermissions.iterator()
        ...
        requestPermissions(
            ...
            object : Runnable {
                override fun run() {
                    ...
                    // 获取第下一次要申请权限列表中的首个权限
                    val firstNextPermission: IPermission? = nextPermissions[0]
                    // 如果下一个请求的权限是后台权限
                    if (firstNextPermission.isBackgroundPermission(activity)) {
                        val foregroundPermissions: List<IPermission> =
                            firstNextPermission.getForegroundPermissions(activity)
                    ...
                    val maxWaitTimeByPermissions: Int =
                        PermissionApi.getMaxIntervalTimeByPermissions(activity, nextPermissions)
    ...
    companion object {
        ...
        private fun getUnauthorizedPermissions(
            ...
            // 未授权的权限列表
            val unauthorizedPermissions: MutableList<List<IPermission?>> =
                ArrayList<List<IPermission?>>(requestPermissions.size)
            // 已处理的权限列表
            val alreadyDonePermissions: MutableList<IPermission?> =
                ArrayList<IPermission>(requestPermissions.size)

            // 遍历需要请求的权限列表
            for (i in requestPermissions.indices) {
                val permission: IPermission = requestPermissions[i]
                ...
                // 查询危险权限所在的权限组类型
                val permissionGroup: String = permission.getPermissionGroup()
                ...
                for (j in i..<requestPermissions.size) {
                    val todoPermission: IPermission = requestPermissions[j]
                    // 如果遍历到的权限对象不是同一个组别的，就继续找
                    if (!PermissionUtils.equalsString(
                            todoPermission.getPermissionGroup(),
                            permissionGroup
                        )
                    ) {
                        continue
                    ...
                    if (todoPermissions == null) {
                        todoPermissions = ArrayList<IPermission?>()
                    }
                    // 添加到待处理的权限列表中
                ...
                val iterator: MutableIterator<IPermission?> = todoPermissions.iterator()
                while (iterator.hasNext()) {
                    val todoPermission: IPermission? = iterator.next()
                    ...
                    // 将后台权限单独领出来放到另外一个集合中
                    iterator.remove()
                    backgroundPermissions = ArrayList<IPermission?>()
        ...
        private fun requestPermissions(
            ...
            val continueRequestRunnable = Runnable {
                fragmentFactory.createAndCommitFragment(
                    permissions,
                    permissionType,
                    object : com.knight.kotlin.library_permiss.core.OnPermissionFlowCallback {
